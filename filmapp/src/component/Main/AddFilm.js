...
const AddFilm = ({title, description, pathImage, popularity, realiseDate, genres, averageVote, voteCount, isAdult, genresMap, changeInput, adultInputChange, validateInputs, genresInputChange}) => {
    return (
        <div className='main'>
            ...
            {genresMap.map(el => <label>{el.name}<input type="checkbox" checked={genres.value.indexOf(el.id) != -1? 'checked': ''} onChange={(ev) => genresInputChange(el.id)}/></label>)}
            ...
            <button onClick={() => validateInputs('addFilm', 'title', 'description', 'pathImage', 'popularity', 'realiseDate', 'genres','averageVote', 'voteCount', 'isAdult')}>Add</button>
            <button onClick={() => validateInputs('clear', 'title', 'description', 'pathImage', 'popularity', 'realiseDate', 'genres','averageVote', 'voteCount', 'isAdult')}>Clear</button>
        </div>
    );
};
